...
- Text Formatting 프로그램 

  - 단어 사이의 공백을 모두 하나로 줄여 주는 프로그램

  - 20개의 문자를 넘는 단어면 20개를 초과하는 부분은 *하나로 치환하는 기능

  프로그램 이름 : justify

  입렵 파일 이름 : quote

  justify프로그램에 quote 파일을 넣어 newquote로 출력
  ...
  void read_word(char *word, int len);   // 다음 단어를 읽고 저장한다. EOF면 비어둔다.
  ...
  void write_line(void)
  {
      ...
      extra_spzces = MAX_LINE_LEN - line_len;
      for(i=0; i< line_len; i++){
          if(line[i] != ' ')
  	        putchar(line[i]);
          else {
              spaces_to_insert - sxtra_spaces / (num_words -1 );
...
- 컴파일링
  ...
  - 하나의 프로그램의 소스파일들은 각각 컴파일 된다.
  - 헤더파일은 컴파일 될 필요가 없다.  헤더파일은 소스파일이 include하는 경우만 컴파일 된다.
  - 각각의 소스파일 마다, 컴파일러는 object code를 포함하는 파일을 생성한다. 이 파일을 object file이라고 한다. UNIX에서는 .o 의 WINDOW에서는 .obj의 확장자를 가진다.

- 링킹

  - 링커는 오브젝트 파일을 라이브러 함수와 함께 합쳐준고 실행가능한 파일을 만들어 준다. 
  - 링커의 의무중 하나는 컴파일러가 남긴 외부의 레퍼런스를 연결해 준다. (외부 레퍼런스는 하나의 파일에 있는 함수가 다른 파일에 있는 함수를 호출하거나 변수에 접근할 때 발생한다.)

- **Makefiles**

  - 모든 소스파일의 이름을 커맨드라인에 입력하는 것은 힘들다.
  - 프로그램을 rebuilding 할 때에도 시간이 오래걸린다.
  - UNIX에서는 makefile의 컨셉을 만들어 쉽게 만들었다.
  - makefile은 프로그램의 부분이되는 파일들을 리스트하는 것 뿐만 아니라, 파일들의 의존상태까지 기술한다. foo.c 파일이 bar.h를 포함하고 있다면, foo.c는 bar.h에 의존한다. 만약, bar.h를 수정한다면, foo.c도 다시 컴파일 되어야 하기 때문에 foo.c는 bar.h에 의존적이라고 할 수 있다.

  - 아래의 4가지 그룹은 각각 rule이라고 불린다. 각 rule의 첫번 쨰 라인은 target 파일과 target파일이 의존하고 있는 파일들이 나열된다. 두번 쨰 줄은 타켓이 rebuild되기 위해 실행되어야 하는 명령어이다. 타겟이 의존하는 파일들이 수정될 때 rebuild가 필요하다. 

    > justify : justify.o word.o line.o   // 타겟 파일 : 타켓의 의존파일
    >
    > ​	gcc -o justify justify.o word.o line.o  // gcc 명령어로 오브젝트파일들을 링킹해줌, -o 옵션은 출력 파일명을 지정하는 옵션이다. 지정하지 않으면 a.out으로 파일을 생성함.

    > justify.o : justify.c word.h line.h  // 타겟 파일 : 타켓의 의존파일
    >
    > ​	gcc -c justify.c   // gcc -c 명령어로 컴파일 해줌
  ...
  - 한번, makefile을 만들면 프로그램을 빌드(또는 rebuild)하는 것에 유용하다. 연관된 파일들의 수정된 날짜와 시간을 확인 하여, make명령어가 어떤 파일이 최신 버전이 아닌지 알 수 있고, 프로그램을 rebuild하는데 필수적인 명령을 수행한다.

  - make규칙

    - makefile에 있는 각각의 command의 앞에는 tab이 있어야 한다.
    - makefile의 파일 명은 보통 Makefile 또는 makefile로 한다. make명령어가 사용될 때 자동으로 현재 디렉토리에서 두 이름중 하나를 찾는다.
    - make 명령어는 아래와 같이 사용한다.
      - make target    // makefile에 기술된 타켓의 리스트 중 하나를 선택한다. 만약 justify를 make하려면 make justify를 하면 된다.
      - make  // target 없이 make를 하면 가장 위ㄴ에 있는 rule의 taget을 build 한다.

  - makefile은 매우 복잡하기 때문에 다 알기 어렵다. makefile대신에 IDE가 제공하는 project files를 쓰는 사람도 많다.

- **Errors During Linking**

  - 컴파일 시에 감지되지 몇몇 에러는 링킹시에 발견된다. 예를들면, 함수나 변수의 정의가 되지 않은 경우 링커가 external referecnce를 연결하지 못하므로, 에러 경고를 보낸다. (uundefuned symbol 또는 undefiend reference)
  - 링커에 의해 감지된 에러는 수정하기 쉬운경우가 많다. 
  - 링킹시 자주 발생하는 에러 유형
    - Misspelling
    - Missing files : makefile에 필요한 파일이 빠져 있는지 체크
    - Missing Libraries : 링커가 모든 라이브러리 함수를 찾기란 어렵다. 단순히 라이브러리를 inlcude하는 것만으로는 충분하지 않을 수 있다.  예를 들면, math.h 라이브러리를 사용하고 싶을 때 -lm 옵션을 써서 프로그램이 링킹될때 명확하게 해주는 것이 좋다.
      (-l 옵션 : 라이브러리를 지정. 파일명과 함께 붙여 쓴다. / -lm 옵션은 수학라이브러리를 포함한다는 것을 지정 / unix에서 라이브러리의 명은 lib~~~.a 형식으로 되어있고, 수학라이브러리는 libm.a 이다. )

- **Rebuilding a Program**

  - 모든 파일을 재 컴파일 하는 경우는 드물다.  수정된 코드와 연관이 있는 파일들만 재 컴파일 해서 시간을 단축 시킬 수 있다. 

  - 만약, word.c와 justify.c 파일이 수정되었고 line.c는 수정되지 않았다면, `gcc -o justify justify.c word.c line.o` 로 표현 가능하다. (line.c가 아니라 line.o라는 것을 주의)

  - makefile을 쓰면 자동으로 컴파일되어야할 파일만 컴파일 해준다. word.h와 word.c와 justify.c만 수정되었다면, make justify는 아래와 같이 수행 될 것이다.

    1. justify.o가 justify.c 컴파일의 결과로 생성됨.
    2. word.o가 word.c 컴파일의 결과로 생성됨
    3. justify가 justify.o와 word.o와 line.o가 링킹되어 생성됨

    

- **Defining Macros Outside a Program**

  gcc 는 -D 옵션으로 define macro를 정의할수 있다.

  ```c
  gcc -DDEBUG=1 foo.c
  ```

  디버깅시 위의 코드를 이용하여 디버그에 이용 할 수 있다.

  -U옵션으로 매크로를 undefine 할 수 있다.  #undef 
...
#### 18.1 Declation Syntax

선언은 컴파일러에게 식별자의 의미를 전달해준다. 

int i;라고 선언하면, 컴파일러에게 현재 scope에서 i는 int형 변수 타입이라는 것을 알려주는 것이다.

float f(float); 라고 함수를 선언하면, 컴파일러에게 f함수는 float타입의 리턴 값과, float타입 한개의 아규먼트를 가진다고 알려주는 것이다.

`declation-specifiers declarators;` 

- **declation-specifiers** : 변수나 함수의 특성을 묘사

  1. Storage classes : 4가지 storage class가 있음. 최대 1개의 storage class사용 가능. 만약 사용한다면 가장 앞에 위치.
     - auto
     - static
     - extern
     - register
  2. Type qualifiers : C89는 const, volatile만 있고, C99에는 restrict 추가. 여러개의 type qualifier 사용 가능
     - const
     - volatile
     - restrict
  3. Type specifiers 
     - void, char, short int , long, float, doulbe, signed, unsigend 등. 순서는 중요하지 않다. (int unsigned long == long unsigned int) 
     - struct, union, enum
     - typedef를 이용해 생성된 타입도 type specifier
  4. function specifirer : C99부터 있음. 함수 선언에만 사용됨. inline 하나만 있음.
     - inline

  > Type qualifier와 type specifier는 Stroage class 뒤에 위치하고, 둘간의 순서는 중요하지 않다. 하지만, type specifier보다 type qulifier를 먼저 쓰는것이 보기 좋다. 

- **declarators** : 특성에 이름이나 추가적인 정보를 부여

  - identifiers : 간단한 변수의 이름
  - identifiers followed  by [] : 배열 이름
  - identifiers preceded by * : 포인터 이름
  - identifiers followd by () : 함수 이름

  > declarator들은 콤마로 구분하고, 하나의 declarator는 선언과 초기화가 동시에 가능하다.

<예시>

```c
static float x, y, *p;
//storage class / type specifier / declarators

const char month[] = "January";
//type qualifier / type specifier / declarator / initializer

extern const unsigned long int a[10];
//storage class / type qualifier / type specifier / declarator

extern int squara(int);
